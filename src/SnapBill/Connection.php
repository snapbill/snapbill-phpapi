...
class Connection {
  ...
  // The HTTP connection to use for posting to the API.
  public $http;
  ...
  function __construct($options=array()) {
    $this->http = is_array($options) ? new Http($options) : $options;
  ...
  function search($class, $search=array()) {
    ...
    $results = $this->http->post("$class/list", $search)['list'];
  ...
  function add($class, $data) {
    $result = $this->http->post("$class/add", $data);
